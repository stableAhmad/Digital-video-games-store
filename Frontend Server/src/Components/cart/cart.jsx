...
export default function Cart() {
  const { cartGames, userData, gameData } = useContext(CartContext);
  ...
  useEffect(() => {
    async function displayCart() {
      let response = await axios.get(`http://localhost:4000/app1/get/relation/${userData}`)
  ...
  async function handleDecrement(gameID) {
    console.log(userData);
    let { data } = await axios.delete(`http://localhost:4000/delete/relation/cart/${userData}/64722f06eb6f99f9f7686a4d`)
    console.log(data);
  }
